...
void fms_rxbit(struct demod_state *s, int bit)
{
    ...
    if ((s->l2.fmsfsk.rxstate & 0x0007FFFF) == 0x7FF1A)
    {
        ...
        // Initialize the running crc to 0s
        for (i=0; i<7; ++i)
        {
            s->l2.fmsfsk.crc[i] = 0;
        }
    ...
    // If we have a valid SYNC, record the message by appending it (from the left) to the RX buffer
        ...
        // The message is one bit longer than the stuff protected by the CRC. The last bit is for stuffing and aligning
        if (s->l2.fmsfsk.rxbitcount < 49)
        {
            ...
            doinvert = bit ^ s->l2.fmsfsk.crc[6];         // XOR required?

            s->l2.fmsfsk.crc[6] = s->l2.fmsfsk.crc[5] ^ doinvert;
            s->l2.fmsfsk.crc[5] = s->l2.fmsfsk.crc[4];
            s->l2.fmsfsk.crc[4] = s->l2.fmsfsk.crc[3];
            s->l2.fmsfsk.crc[3] = s->l2.fmsfsk.crc[2];
            s->l2.fmsfsk.crc[2] = s->l2.fmsfsk.crc[1] ^ doinvert;
            s->l2.fmsfsk.crc[1] = s->l2.fmsfsk.crc[0];
            s->l2.fmsfsk.crc[0] = doinvert;
        }
